...
function removeProperties(obj, keys) {
  ...
  keys.forEach((key) => {
    if (Object.keys(copyOfObj).includes(key)) {
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    const banknote = queue[i];

    switch (banknote) {
      case 25:
        available[banknote] += 1;
        break;
      case 50:
        if (available[25] >= 1) {
          available[25] -= 1;
        ...
        break;
      case 100:
        if (available[25] >= 1 && available[50] >= 1) {
          available[25] -= 1;
          available[50] -= 1;
          available[banknote] += 1;
        } else if (available[25] >= 3) {
          available[25] -= 3;
          available[banknote] += 1;
        } else {
          return false;
...
function sortCitiesArray(arr) {
  ...
  return sortedByCountry.sort((a, b) => {
